...
- Focused is an explicit Luck-layer input with baseline 1. The only existing Focused source was retired Lantern Focused Beam. A future valid source can populate that input. Existing playtime Luck is retained in the special layer. World effects apply last, including to one-roll potions; the old additive admin Luck setting is converted into an equivalent final world factor for debugging.
