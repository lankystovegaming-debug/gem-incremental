...
5. Apply `supabase/migrations/20260925103659_rebalance_deepcore_route_specimen_requirements.sql`.
6. Deploy the updated `deepcore` Edge Function.
7. Deploy the updated `roll` Edge Function.
...
The fifth migration reduces the Crystalline Passage requirement from 4,000 to 1,000 Exotic+ specimens. It immediately reconciles the locked route state, so a route that already satisfies both its funding and specimen requirements resolves exactly once when the migration is applied.
