...
mountShell({ page: "gem-fusion", base: "../" });
...
const [intro,config] = details["gem-fusion"];
...
async function load() {
  try {
    ...
    const section = (data.sections || []).find(s => s.id === "gem-fusion");
    const enabled = section?.enabled === true;
    $("featureState").textContent = enabled ? "LIVE" : "OFF";
    $("featureState").className = `state-pill ${enabled ? "on" : "off"}`;
    const defs = data.definitions?.filter(d => d.feature_type === "gem-fusion") || [];
