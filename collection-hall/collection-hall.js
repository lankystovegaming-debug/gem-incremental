...
const [intro,config] = details["collection-hall"];
...
async function load() {
  try {
    ...
    const section = (data.sections || []).find(s => s.id === "collection-hall");
    const enabled = section?.enabled === true;
    $("featureState").textContent = enabled ? "LIVE" : "OFF";
    $("featureState").className = `state-pill ${enabled ? "on" : "off"}`;
    const defs = data.definitions?.filter(d => d.feature_type === "collection-hall") || [];
