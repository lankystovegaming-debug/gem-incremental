...
export function mountEquipmentLoadouts(host, refreshEquipment) {
 ...
 function paint() {
  host.innerHTML = `<div class="loadout-manager__head"><div><h2>Equipment Loadouts</h2><p>Five named presets. Missing items stay unchanged; empty saved slots are unequipped.</p></div></div><div class="loadout-manager__list">` + Array.from({length:5},(_,i)=>{
   ...
   return `<article class="card loadout-row"><label class="loadout-row__name"><span>Preset ${slot}</span><input class="input" data-name="${slot}" maxlength="40" value="${escapeHtml(preset?.name??'')}" placeholder="Loadout ${slot}" aria-label="Loadout ${slot} name"></label><p class="loadout-row__details">${escapeHtml(details)}</p><div class="loadout-row__actions"><button class="btn btn--sm" data-action="save" data-slot="${slot}" ${busy?'disabled':''}>${preset?'Update':'Save current'}</button>${preset?`<button class="btn btn--primary btn--sm" data-action="equip" data-slot="${slot}" ${busy?'disabled':''}>Equip</button><button class="btn btn--sm" data-action="delete" data-slot="${slot}" ${busy?'disabled':''}>Delete</button>`:''}</div></article>`;
  }).join('') + '</div>';
 }
