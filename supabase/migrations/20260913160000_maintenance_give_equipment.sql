-- =========================================================
-- MAINTENANCE (DEV) PANEL — EQUIPMENT GRANTS
--
-- Adds an `equipment` action to the existing, server-gated maintenance
-- RPC so a maintainer can grant any equipment item (pickaxe, toy, clover,
-- lantern, boots, bag, and any future category) to a target. The granted
-- item is equipped and any other item in the same category is stored,
-- mirroring how crafting settles a category. Bonus columns match
-- player_equipment; overhauled gear is re-normalised from the authoritative
-- recipe by the existing normalize_overhauled_equipment trigger, so the
-- values passed here are only used for pre-overhaul items.
-- =========================================================
