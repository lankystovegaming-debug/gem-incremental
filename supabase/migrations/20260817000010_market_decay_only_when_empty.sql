-- =========================================================
-- Market price decay only applies when nobody owns shares.
--
-- Holding shares freezes the quote between trades. Once the final
-- shareholder sells, the empty market resumes its six-hour half-life
-- decay toward the $1 floor.
-- =========================================================

alter table public.market_state
  add column if not exists has_holders boolean not null default false;

-- Preserve the effective quote players see right now before freezing a
-- market that already has holders. This avoids an artificial jump back to
-- the older stored price at rollout.
...
set
  price = case
    when exists (
      ...
      greatest(
        1::numeric,
        1::numeric + (price - 1::numeric) * power(
          0.5,
          greatest(
            0,
            extract(epoch from (now() - coalesce(decay_updated_at, updated_at, now())))
          ) / (6 * 60 * 60)
        )
      ),
      4
  ...
  decay_updated_at = case
    when exists (
      select 1
      from public.player_shares
      where shares > 0
    ) then now()
    else decay_updated_at
  end,
  has_holders = exists (
    select 1
    from public.player_shares
...
begin
  ...
  -- Serializing trades on the market row makes this global holder check
  -- consistent with the price used for the trade.
  ...
  -- Recheck after the trade so the last seller starts empty-market decay,
  -- while the first buyer immediately freezes the price again.
  select exists (
    select 1
    from public.player_shares
    where shares > 0
