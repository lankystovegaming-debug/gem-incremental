-- Lanterns now grant Mutation Luck (a mutation-chance multiplier) instead of
-- Roll Speed, and the first tier is gated behind a tier-5+ pickaxe.
--
-- This adds a mutation_luck_bonus column to player_equipment, teaches the
-- craft RPC to (a) enforce a new `equipment-min-tier` requirement and (b) store
-- the reward's mutationLuck bonus. The recipe catalogue itself lives in
-- public.game_recipes and is reseeded from src/data/recipes.js via the
-- seed-recipes function — run that after this migration so the lantern rows
-- carry `bonus.mutationLuck` and the pickaxe requirement. The roll Edge
-- Function reads mutation_luck_bonus and applies it to mutation chance.
...
begin
  ...
  loop
    if v_req->>'type'='equipment' then
      ...
      if not exists(select 1 from public.player_equipment
        where player_id=v_uid and category=v_req->>'category'
          and tier>=coalesce((v_req->>'tier')::integer,1))
      then raise exception 'requirements_not_met'; end if;
  ...
  insert into public.player_equipment(
    ...
    weight_luck_bonus,weight_multiplier_bonus,mutation_luck_bonus,equipped
  ) values(
    ...
    coalesce((v_bonus->>'mutationLuck')::double precision,0),true
    ...
    mutation_luck_bonus=excluded.mutation_luck_bonus,equipped=true;
