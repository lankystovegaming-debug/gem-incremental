-- =========================================================
-- Idle market-price decay.
--
-- The market price gradually eases toward the $1 floor while there are
-- no new buys. A 24-hour half-life makes the change visible but gradual:
-- each day without new buying halves the remaining distance to the floor.
-- All actual buys and sells still move the price immediately.
-- =========================================================

alter table public.market_state
  add column if not exists decay_updated_at timestamptz;

update public.market_state
set decay_updated_at = coalesce(decay_updated_at, now())
where id = 'coin';
...
declare
  ...
  v_idle_half_life_seconds numeric := 24 * 60 * 60;
...
begin
  ...
  -- Decay only advances from the last stored quote. The client mirrors this
  -- calculation while idle; this locks the true server price at trade time.
