alter table public.player_equipment
  add column if not exists masterwork_level integer not null default 0 check (masterwork_level between 0 and 5),
  add column if not exists masterwork_passive text,
  add column if not exists masterwork_passive_rank integer not null default 0 check (masterwork_passive_rank between 0 and 2),
  add column if not exists masterwork_attunement text,
  add column if not exists masterwork_rerolls integer not null default 0,
  add column if not exists masterwork_choices text[],
  add column if not exists masterwork_perfected_at timestamptz;
...
begin
  ...
  if p_action = 'convert_relics' then
    ...
    v_money_mult := case when v_item.tier >= 13 then 2 when v_item.tier = 12 then 1.6 when v_item.tier = 11 then 1.25 else 1 end;
    ...
    if p_action = 'upgrade' then
      ...
      v_money := (array[1000000,2500000,6000000,15000000,40000000])[v_next] * v_money_mult;
