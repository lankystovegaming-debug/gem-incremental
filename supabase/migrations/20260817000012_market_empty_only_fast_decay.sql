-- =========================================================
-- Rapid empty-market reset.
--
-- Any positive share holding freezes the price. Once the final
-- shareholder sells, the remaining distance to the $1 floor halves
-- every hour until a buyer returns.
-- =========================================================
...
declare
  ...
  v_idle_half_life_seconds numeric := 60 * 60;
...
begin
  ...
  from public.player_shares
  where shares > 0;
  v_has_holders := v_holder_count > 0;
  ...
  -- The final holder's sale starts the fast reset; the first buyer stops it.
  select count(*)::integer
  into v_holder_count
  from public.player_shares
  where shares > 0;
