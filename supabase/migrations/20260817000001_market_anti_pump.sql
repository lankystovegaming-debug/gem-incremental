-- =========================================================
-- Emergency market circuit breaker.
--
-- The previous market charged every share at the pre-trade price but
-- moved the quoted price afterwards. A large buy could therefore lift
-- the quote far above its average purchase price, then a sell could
-- realise that artificial jump. This version uses an exponential curve
-- and charges the integral (average) price along that curve instead.
-- A round trip always loses the trading fee, so pumping cannot mint money.
-- =========================================================
...
declare
  ...
  v_hold_cap bigint := 1000000;
  v_cooldown interval := interval '3 seconds';
  v_baseline numeric := 10;
  v_floor numeric := 1;
  v_ceiling numeric := 100;
  v_curve_depth numeric := 1000000;
  v_revert numeric;
...
begin
  ...
  -- Serialise all trades before calculating a curve price.
  select price, updated_at
  into v_price, v_updated
  from public.market_state
  where id = 'coin'
  for update;

  -- Keep the normal time-based drift, then apply the trade to the curve.
  v_revert := power(0.5, extract(epoch from (now() - v_updated)) / (15 * 60));
  v_price := greatest(v_floor, least(v_ceiling,
    v_baseline + (v_price - v_baseline) * v_revert));
  ...
  if p_action = 'buy' then
    ...
    -- Integral of price(x) = price(0) * exp(x / depth).
    ...
    -- The reverse integral pays the same curve value, less the sell fee.
...
-- The old quote may already be manipulated. Reset it once while preserving
-- player shares; future trades are protected by the curve above.
update public.market_state
