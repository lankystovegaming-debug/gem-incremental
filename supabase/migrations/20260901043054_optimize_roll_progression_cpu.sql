-- Reduce per-roll achievement CPU and WAL pressure.
--
-- The incremental engine previously initialized, locked, evaluated, and
-- updated every enabled definition for every roll. Most achievement
-- definitions listen for a different explicit event type, so that work could
-- never change their progress. Restrict the hot loop to definitions that can
-- accept the incoming event, and avoid writes when the incremental state did
-- not change.
