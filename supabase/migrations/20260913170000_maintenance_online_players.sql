-- =========================================================
-- MAINTENANCE (DEV) PANEL — ONLINE PLAYER LIST
--
-- Adds a read-only `online` action to the server-gated maintenance RPC:
-- the usernames of players seen in the last two minutes, so the CLI can
-- autocomplete a target from who is actually online (the browser cannot
-- read other players' presence directly under the deny-by-default guard).
-- Recreates the whole function to keep every existing action intact.
-- =========================================================
...
begin
  ...
  -- Read-only: usernames of players active in the last two minutes, for the
  -- target picker's autocomplete. No target needed.
  if p_action = 'online' then
    return (
      select coalesce(jsonb_agg(p.username order by pr.last_seen_at desc), '[]'::jsonb)
  ...
  if p_action = 'metric' then
  ...
  elsif p_action = 'equipment' then
    -- Grant a piece of equipment. The item is equipped and any other item
    -- already equipped in the same category is stored, matching how crafting
    -- resolves a category so the target never has two equipped items at once.
    -- Overhauled gear has its bonus columns re-normalised by the
    -- normalize_overhauled_equipment trigger from the authoritative recipe.
    v_equipment_id := p_payload->>'equipment_id';
    v_category := p_payload->>'category';

    if v_equipment_id is null or btrim(v_equipment_id) = ''
       or v_category is null or btrim(v_category) = '' then
      raise exception 'invalid_equipment';
    end if;

    insert into public.player_equipment (
      player_id, equipment_id, category, tier, name,
      luck_bonus, roll_speed_bonus, weight_luck_bonus,
      weight_multiplier_bonus, mutation_chance_bonus, equipped
    ) values (
      v_target,
      v_equipment_id,
      v_category,
      coalesce((p_payload->>'tier')::integer, 1),
      p_payload->>'name',
      coalesce((p_payload->>'luck_bonus')::double precision, 0),
      coalesce((p_payload->>'roll_speed_bonus')::double precision, 0),
      coalesce((p_payload->>'weight_luck_bonus')::double precision, 0),
      coalesce((p_payload->>'weight_multiplier_bonus')::double precision, 0),
      coalesce((p_payload->>'mutation_chance_bonus')::double precision, 0),
      true
    )
    on conflict (player_id, equipment_id) do update
      set category = excluded.category, tier = excluded.tier, name = excluded.name,
          ...
          mutation_chance_bonus = excluded.mutation_chance_bonus, equipped = true;
    ...
    v_result := jsonb_build_object('equipment', v_equipment_id, 'category', v_category);
