...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    const [profileResult, exhibitsResult, gemsResult, definitionsResult, registrationsResult, completionsResult, playerResult] = await Promise.all([
      ...
      ctx.supabaseAdmin.from("inventory_gems").select("id,gem_name,rarity,mutation_chance_multiplier,base_weight,final_weight,value,mutation_ids,serial_number,locked,museum_locked,created_at").eq("player_id", playerId).neq("gem_name", "Enchant Relic").neq("gem_name", "Ancient Relic").order("rarity", { ascending: false }).limit(500),
