...
export default {fetch:withSupabase({auth:"user"},async(req,ctx)=>{if(req.method==="OPTIONS")return new Response("ok",{headers:cors}); const userId=uid(ctx); if(!userId)return json({error:"unauthenticated"},401); let b:any={};try{b=await req.json()}catch{} const a=b.action;
 try{
  ...
  if(a==="guild-create"){const name=String(b.name??"").trim().slice(0,50);if(name.length<2)return json({error:"invalid_name"},400);const {data:existing}=await ctx.supabaseAdmin.from("guild_members").select("guild_id").eq("player_id",userId).maybeSingle();if(existing)return json({error:"already_in_guild"},409);const g=await ctx.supabaseAdmin.from("guilds").insert({name,owner_id:userId}).select().single();if(g.error)throw g.error;const m=await ctx.supabaseAdmin.from("guild_members").insert({guild_id:g.data.id,player_id:userId,role:"owner"});if(m.error)throw m.error;return json({guild:g.data});}
