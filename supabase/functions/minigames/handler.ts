...
const response = (v: any, status = 200) =>
...
export function createHandler(admin: any, clock = () => Date.now()) {
  return async (req: Request) => {
    ...
    try {
      ...
      if (auth?.error || !auth?.data?.user || auth.data.user.is_anonymous)
        return response({ error: "Sign in to play minigames." }, 401);
      const user = auth.data.user.id;
      const [pr, br] = await Promise.all([
        admin.from("players").select("id").eq("id", user).maybeSingle(),
        admin
      ...
      check(unwrap(pr), "Open Roll to finish creating your profile.");
      const ban = unwrap(br);
      check(
        !ban?.active_until || Date.parse(ban.active_until) <= clock(),
        "Account suspended.",
      );
      const raw = await req.text();
      check(raw.length <= 64000, "Request too large");
      ...
      if (b.action === "start") {
        ...
        run = row(
          await admin.rpc("minigame_start", {
            p_player: user,
            p_game: b.game,
            p_mode: b.mode,
            p_state: state,
      ...
      const wallet = row(
        await admin.rpc("minigame_wallet", { p_player: user }),
      );
      if (b.action === "state") {
        const rows = unwrap(
          await admin
      ...
      const board =
        b.game && b.game !== "crystal-bags"
          ? unwrap(
              await admin.rpc("minigame_board", {
                p_game: b.game,
                p_player: user,
              }),
            )
      ...
      if (b.game === "crystal-bags") {
        stats = unwrap(
          await admin.rpc("minigame_bag_stats", { p_player: user }),
        );
      }
      return response({
        wallet,
      ...
      return response(
        { error: e instanceof Error ? e.message : "Minigames unavailable" },
        400,
      );
    }
  };
