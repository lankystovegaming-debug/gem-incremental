...
// Workbench is intentionally restricted to the owner/admin allow-list while it is being tested.
const WORKBENCH_OWNER_USER_IDS = ["38d5e8ce-18af-46d3-aa9e-6e601e75dd78"];

async function isWorkbenchAdmin(ctx:any, userId:string){
  if (WORKBENCH_OWNER_USER_IDS.includes(userId)) return true;
  const {data,error}=await ctx.supabaseAdmin.from("admins").select("user_id").eq("user_id",userId).maybeSingle();
  if(error){ console.error("WORKBENCH_ADMIN_LOOKUP",error.message); return false; }
...
export default {fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 try{
  const allowed=await isWorkbenchAdmin(ctx,playerId);
  if(!allowed)return json({error:"workbench_admin_only",message:"Workbench is currently available to administrators only."},403);
  const c=await getConfig(ctx);if(!c.enabled)return json({error:"feature_disabled",message:"Workbench is currently disabled."},403);
