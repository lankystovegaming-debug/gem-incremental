...
export async function consoleHarness(kind, overrides = {}) {
  ...
  const bindings = { ...ui, ...format, ...crystal, ...rpc, document, mountShell: () => ({ setWallet() {} }), ensurePlayerAuth: async () => false, loadCloudPlayerState: async () => ({ money: 10000000 }), startExpeditionRefresh() {}, notify: { error: (...x) => notices.push(x), success: (...x) => notices.push(x) }, confirmDialog: async options => { dialogs.push(options); return "cancel"; }, setInterval() {}, alert: message => notices.push(message), ...overrides };
