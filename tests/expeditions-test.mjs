...
assert.match(page,/Secure Current Cargo/);assert.match(page,/Full Resupply/);assert.doesNotMatch(page,/Build Supply Camp|run\.depth\*100000/);assert.match(page,/dashboard\.checkpointServices/);assert.match(page,/run\.status==="checkpoint_decision"/);assert.match(page,/Enter Overdepth/);assert.match(page,/Extract voluntarily/);
...
assert.match(improvements,/unique index abandoned_mine_funding_overdepth_once/);assert.match(html,/View Loot Table/);assert.match(html,/lootTableDialog/);assert.match(page,/Spend \$\{formatMoney\(cost\)\} and enter/);assert.match(page,/permanently spent/);assert.match(page,/Danger:[\s\S]*projectedDanger/);assert.match(page,/incident can occur during entry/);assert.match(page,/money<dashboard\.nextOverdepthCost/);
assert.match(html,/How expeditions work/);for(const detail of ["Fund a depth","Make genuine rolls","Manage the descent","Extract and settle","Supply Camps at D3, D6, and D9","Museum artifacts are protected","After D10"])assert.match(html,new RegExp(detail));
