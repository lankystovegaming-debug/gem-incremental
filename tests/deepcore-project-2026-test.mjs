...
await db.exec("update deepcore_event_state set phase=52,crystalline_funding=0,crystalline_specimens=0,anomalous_funding=0,anomalous_specimens=0,route_winner=null");
await db.query("select deepcore_set_auto_contribute(true,100000,null,null,'crystalline')");
let auto=await one("select auto_contribute,auto_route from deepcore_players where player_id=$1",[uid]);
assert.equal(auto.auto_contribute,true); assert.equal(auto.auto_route,"crystalline","Phase 5.2 auto contribution stores one explicit passage");
await db.query("select set_config('request.jwt.claims',$1,false)",[JSON.stringify({role:"service_role"})]);
let automatic=(await one("select deepcore_auto_contribute_roll($1,$2::jsonb) result",[uid,JSON.stringify({gem_name:"Route Quartz",rarity:100000,value:50,final_weight:10,rolled_weight_multiplier:1,mutation_ids:[]})])).result;
assert.equal(automatic.contributed,true); assert.equal(automatic.objective,"crystalline");
assert.equal(Number((await one("select crystalline_specimens from deepcore_event_state")).crystalline_specimens),1,"Crystalline auto-routing advances the selected passage");
assert.equal((await one("select auto_contribute from deepcore_players where player_id=$1",[uid])).auto_contribute,true,"route auto-contribution remains enabled while Phase 5.2 is unresolved");

await db.query("select set_config('request.jwt.claims',$1,false)",[JSON.stringify({role:"authenticated"})]);
await db.query("select deepcore_set_auto_contribute(true,1000000,null,null,'anomalous')");
await db.query("select set_config('request.jwt.claims',$1,false)",[JSON.stringify({role:"service_role"})]);
automatic=(await one("select deepcore_auto_contribute_roll($1,$2::jsonb) result",[uid,JSON.stringify({gem_name:"Ordinary Million",rarity:1000000,value:75,final_weight:10,rolled_weight_multiplier:1,mutation_ids:[]})])).result;
assert.equal(automatic.contributed,false,"Anomalous auto-routing rejects non-special specimens");
await db.exec("update private_feature_gems set special_gem=true where name='Blacksite Crystal'");
automatic=(await one("select deepcore_auto_contribute_roll($1,$2::jsonb) result",[uid,JSON.stringify({gem_name:"Blacksite Crystal",rarity:1000000,value:75,final_weight:10,rolled_weight_multiplier:1,mutation_ids:[]})])).result;
assert.equal(automatic.contributed,true); assert.equal(automatic.objective,"anomalous");
...
const crateRequest="70000000-0000-0000-0000-000000000001";
const crateResult=(await one("select deepcore_open_crate($1) result",[crateRequest])).result;
assert.ok(crateResult.primary?.id); assert.ok(Number(crateResult.primary?.quantity)>0); assert.equal(typeof crateResult.rollCard,"boolean","crate responses contain displayable reward contents");
const crateReplay=(await one("select deepcore_open_crate($1) result",[crateRequest])).result;
assert.deepEqual(crateReplay,crateResult,"crate response retries return the same contents");
