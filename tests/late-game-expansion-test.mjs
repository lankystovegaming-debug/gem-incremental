...
assert.equal(recipes.filter(r=>r.id==='bottomless-singularity').length,0);
...
assert.match(source,/equipmentContext.flags.surge/);
assert.match(source,/genuineRoll % 67 === 0 && random01\(\) < 1 \/ 67/);
assert.match(source,/enchant: enchantLuck \+ \(alignmentRoll \? 50 : 0\)/);
assert.match(source,/eternityRoll\) mutationChanceMultiplier \+= 50/);
assert.doesNotMatch(source,/finalWeight \/ gem\.baseWeight >= 5 \? 1\.10 : 1/);
