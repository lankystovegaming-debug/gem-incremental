...
const volcanicCss = read("volcanic-depths/volcanic-depths.css");
...
// ── Volcanic Depths CSS polish + bug fixes ────────────────────────────

// Bug fix: the progress bar markup is `.progress-track > i`, which had no
// styling at all (invisible fill). The track and its molten fill must both
// be styled now.
assert.match(volcanicCss, /\.progress-track\s*\{[\s\S]*?background/,
  ".progress-track must have a visible groove");
assert.match(volcanicCss, /\.progress-track\s*>\s*i\s*\{[\s\S]*?background:\s*linear-gradient/,
  ".progress-track fill must render a molten gradient bar");

// Gap fix: the equipment panel (.console-panel) was referenced by the page
// but never styled.
assert.match(volcanicCss, /\.console-panel\s*\{/,
  ".console-panel must be styled");

// Gap fix: the page emits `eruption` and `unknown` states, which had no
// matching modifier styles.
assert.match(volcanicCss, /\.volcanic-state--eruption\b/,
  "the active-eruption state must be styled");
assert.match(volcanicCss, /\.volcanic-state--unknown\b/,
  "the unknown-threshold state must be styled");

// Theme-awareness: no hard-coded white/black alpha surfaces (they break in
// light theme); colours must derive from tokens / color-mix.
assert.doesNotMatch(volcanicCss, /rgba\(255,\s*255,\s*255/,
  "volcanic CSS must not hard-code white alpha surfaces");
assert.match(volcanicCss, /@media \(prefers-reduced-motion: reduce\)/,
  "volcanic CSS must respect reduced motion");

// Bug fix on the board: buttons live inside .destination__actions, so that
// wrapper (the real flex child) must be the one pushed to the card bottom.
