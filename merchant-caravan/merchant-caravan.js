...
mountShell({ page: "merchant-caravan", base: "../" });
...
const [intro,config] = details["merchant-caravan"];
...
async function load() {
  try {
    ...
    const section = (data.sections || []).find(s => s.id === "merchant-caravan");
    const enabled = section?.enabled === true;
    $("featureState").textContent = enabled ? "LIVE" : "OFF";
    $("featureState").className = `state-pill ${enabled ? "on" : "off"}`;
    const defs = data.definitions?.filter(d => d.feature_type === "merchant-caravan") || [];
