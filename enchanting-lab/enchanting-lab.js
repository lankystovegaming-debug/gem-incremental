...
mountShell({ page: "enchanting-lab", base: "../" });
...
const [intro,config] = details["enchanting-lab"];
...
async function load() {
  try {
    ...
    const section = (data.sections || []).find(s => s.id === "enchanting-lab");
    const enabled = section?.enabled === true;
    $("featureState").textContent = enabled ? "LIVE" : "OFF";
    $("featureState").className = `state-pill ${enabled ? "on" : "off"}`;
    const defs = data.definitions?.filter(d => d.feature_type === "enchanting-lab") || [];
